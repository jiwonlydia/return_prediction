...
# pip install wrds
# pip install pyarrow # in terminal
...
# link comp and crsp
comp['ncusip'] = comp['cusip'].apply(lambda x: x[:8])
crsp2 = crsp2.rename(columns={'monthend': 'jdate'})
...
# agr
data_rawa['at_l1'] = data_rawa.groupby(['gvkey'])['at'].shift(1)
...
data_rawa['chato'] = (data_rawa['sale']/((data_rawa['at']+data_rawa['at_l1'])/2))-                     (data_rawa['sale_l1']/((data_rawa['at']+data_rawa['at_l2'])/2))
...
#ccm1 = pd.merge(comp, ccm, how='left', on=['gvkey'])
comp['yearend'] = comp['datadate'] + YearEnd(0)
comp['jdate'] = comp['datadate'] + MonthEnd(4)  # we change quarterly lag here
...
'''
comp['ibq'] = np.where(comp['ibq_diff'] >= 0, comp['ibq_new'], comp['ibq_old'])
comp['ibq'] = np.where(comp['ibq'].isnull(), comp['ibq_old'], comp['ibq'])  # for most recent record we can only use the lag-4-months ibq
...
data_rawq['noa'] = (data_rawq['atq']-data_rawq['cheq']-data_rawq['ivaoq'])-                 (data_rawq['atq']-data_rawq['dlcq']-data_rawq['dlttq']-data_rawq['mibq']-data_rawq['pstkq']-data_rawq['ceqq'])/data_rawq['atq_l4']
...
data_rawq['ala'] = data_rawq['cheq'] + 0.75*(data_rawq['actq']-data_rawq['cheq'])+                 0.5*(data_rawq['atq']-data_rawq['actq']-data_rawq['gdwlq']-data_rawq['intanq'])
...
data_rawq['cinvest'] = ((data_rawq['ppentq'] - data_rawq['ppentq_l1']) / data_rawq['saleq'])                       -(data_rawq[['c_temp1', 'c_temp2', 'c_temp3']].mean(axis=1))
...
data_rawq['pscore'] = data_rawq['p_temp1']+data_rawq['p_temp2']+data_rawq['p_temp3']+data_rawq['p_temp4']                      +data_rawq['p_temp5']+data_rawq['p_temp6']+data_rawq['p_temp7']+data_rawq['p_temp8']                      +data_rawq['p_temp9']
